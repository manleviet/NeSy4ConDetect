...
class SingleConflictData:
    def __init__(self, dpath, kbpath=None, batch_size=16, nfold=None):
        """
        Use leave-one-out
        """
        self.X, self.Y = load_single_conflict_data(dpath)
        self.nvar = self.X.shape[1]
        if kbpath is not None:
            self.kb = load_knowledge(kbpath)
...
def load_knowledge(kbpath) -> DIMACSReader:
    # fs = "../data/busybox/kb/busybox.dimacs"
    dr = DIMACSReader(kbpath)
    return dr
