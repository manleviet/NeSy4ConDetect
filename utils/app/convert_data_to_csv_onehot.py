...
CONFLICTS_OUTPUT_PATH = '%s/conflicts_onehot.csv' % ROOT_PATH
INVALID_CONFS_OUTPUT_PATH = '%s/invalid_confs_onehot.csv' % ROOT_PATH
...
for invalid_conf_file, conflict_file in paths:
    invalid_conf_vector = convert_conf_to_vector(invalid_conf_file, feature_map, conf_id, 1)
    invalid_conf_vectors.append(invalid_conf_vector)

    conflict_vector = convert_conflict_to_vector_onehot(0, conflict_file, feature_map, conf_id, 1)
...
for invalid_conf_file, conflict_file in paths:
    ...
    with open(conflict_file, 'r') as file:
        for j, line in enumerate(file):
            ...
            if j >= 5:
                # print(line.strip())
                conflict_vector = convert_conflict_to_vector_onehot(1, line, feature_map, conf_id, 1)
